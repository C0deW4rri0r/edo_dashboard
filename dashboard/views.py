...
def dashboard_page(request):
    ...
    logger.info('Загрузка страницы дашборда')
...
def kpi_cards(request):
    ...
    try:
        ...
        logger.info(f'KPI данные: total={total}, on_approval={on_approval}, overdue={overdue}, avg_days={avg_days}')
        ...
        logger.error(f'Ошибка при получении KPI данных: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Ошибка при загрузке данных'}, status=500)

@cache_page(300)
def approval_funnel(request):
    """API: Воронка согласования (количество по статусам)"""
    try:
        logger.info('Запрос данных воронки согласования')
        ...
        logger.info(f'Воронка согласования: {len(labels)} статусов')
        ...
        logger.error(f'Ошибка при получении данных воронки: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Ошибка при загрузке данных'}, status=500)
...
def monthly_dynamics(request):
    ...
    try:
        logger.info('Запрос данных динамики по месяцам')
        ...
        logger.info(f'Динамика по месяцам: {len(labels)} месяцев')
        ...
        logger.error(f'Ошибка при получении данных динамики: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Ошибка при загрузке данных'}, status=500)
...
def contract_types(request):
    ...
    try:
        logger.info('Запрос данных по типам договоров')
        ...
        logger.info(f'Типы договоров: {len(labels)} типов')
        ...
        logger.error(f'Ошибка при получении данных по типам: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Ошибка при загрузке данных'}, status=500)
...
def slowest_contracts(request):
    ...
    try:
        ...
        logger.error(f'Ошибка при получении данных о медленных согласованиях: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Ошибка при загрузке данных'}, status=500)